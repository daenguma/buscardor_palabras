USE webuser_bit
go
REVOKE EXECUTE ON dbo.sp_login_web FROM [roleBancaMovil ]
go
IF OBJECT_ID('dbo.sp_login_web') IS NOT NULL
BEGIN
    DROP PROCEDURE dbo.sp_login_web
    IF OBJECT_ID('dbo.sp_login_web') IS NOT NULL
        PRINT '<<< FAILED DROPPING PROCEDURE dbo.sp_login_web >>>'
    ELSE
        PRINT '<<< DROPPED PROCEDURE dbo.sp_login_web >>>'
END
go
...
go
IF OBJECT_ID('dbo.sp_login_web') IS NOT NULL
    PRINT '<<< CREATED PROCEDURE dbo.sp_login_web >>>'
ELSE
    PRINT '<<< FAILED CREATING PROCEDURE dbo.sp_login_web >>>'
go
GRANT EXECUTE ON dbo.sp_login_web TO [roleBancaMovil ]
go
